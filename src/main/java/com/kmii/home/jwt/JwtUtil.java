...
public class JwtUtil {
	...
	@Value("${jwt.secret}")
	private String secret;  //application.properties 내에 선언되어 있는 jwt.secret 가져와 저장
	...
	public String generateToken(String username) {
		...
		return Jwts.builder()
				.setSubject(username)  // 인증받을 사용자 이름
	...
	public String extractUsername(String token) {
		return Jwts.parser()  // 토큰을 parsing 해주는 parser 생성
				.setSigningKey(secret)  // 서명이 맞는지 검증용 비밀키 설정
				.parseClaimsJws(token)  // 토큰 문자열 분석 -> 서명이 맞는지 검증
				.getBody()  //patload 부분 가져옴
				.getSubject();  // 사용자이름(username) 추출
	}
